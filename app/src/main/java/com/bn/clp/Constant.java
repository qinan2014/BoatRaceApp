...
import com.aihelp.boatraceapp.R;

import java.util.Date;
import java.util.Objects;

/**
 * Created by Boboqi on 2017-9-18.
 */

public class Constant {
    // Have you break the hight record
    public static boolean isBreakRecord;
    // The length of every unit in grass
    ...
    public static String getCurrentTime()
    {
        ...
        String month = d.getMonth() + 1 < 10 ? "0" + (d.getMonth() + 1) : d.getMonth() + "";
    ...
    // How long did you use playing this game
    ...
    // 游戏模式选择的相关标志位
    public static boolean isSpeedmode; // true则为竞速模式，false则为计时模式
    // 小雷达中其他船的位置
    public static float[][] other_Boat_XZ;
    // 为仪表板进行自适应的常量数据 0号为480x800 1号为480x960 3号为320x480
    ...
    public static boolean halfFlag = false; // 是否行使半圈的标志位
    ...
    // 小船速度为0时，小船前后欢动的最大角度值
    public static final float head_Angle_Max = 4.25f;
    // 小船速度为0时，小船前后晃动值的加速度
    public static final float head_Angle_A = 0.25f;
    // 检测在船行使时，速度是否大于该值，当大于该值时，将碰撞标志位置为false表示可以进行下一次的碰撞声音的播放
    public static final float CURR_BOAT_V_PZ = 0.3f;
    // 水面换帧线程工程标志位
    ...
    // 摄像机初始化的视线方向，0表示为z轴负方向
    public static final float DIRECTION_INIT = 0;
    // 摄像机距离所观察模板点的距离
    static final float DISTANCE = 4.0f;
    // 摄像机初始时响应观察点的y坐标
    static final float CAMERA_INI_Y = 2;
    // 小船初始XZ坐标
    public static float YACHT_INI_X = 2.0f;
    public static float YACHT_INI_Z = UNIT_SIZE / 2 + UNIT_SIZE;
    // 纹理图片的宽度
